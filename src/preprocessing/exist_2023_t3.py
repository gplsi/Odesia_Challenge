    ...
    # Flatten the nested lists in "value"
        ...
        return {
            ...
            "IDEOLOGICAL-INEQUALITY": (
                round((ideological_count / total), 4) if total > 0 else 0
            ),
            "NO": round((no_count / total), 4) if total > 0 else 0,
            "STEREOTYPING-DOMINANCE": (
                round((stereotyping_count / total), 4) if total > 0 else 0
            ),
            "SEXUAL-VIOLENCE": (
                round((sexual_violence_count / total), 4) if total > 0 else 0
            ),
            "OBJECTIFICATION": (
