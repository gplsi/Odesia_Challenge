...
def create_gold(partition, base_folder):
    ...
    # Recorrer las carpetas que nos interesan
    folder_files = os.listdir(base_folder)
    
    for folder in folder_files:
        for keyword in GOLDEN_FOLDERS:
            if keyword in folder:
                golden_folder = folder
                golden_files = os.listdir(f"{base_folder}/{golden_folder}")
                
                for file_name in golden_files:
                    if partition in file_name and file_name.endswith('.json'):
                        file_path = os.path.join(base_folder, golden_folder, file_name)
                        with open(file_path, "r", encoding="utf-8") as file:
                            data = json.load(file)
